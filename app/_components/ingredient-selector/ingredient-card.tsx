...
const IngredientCard = React.memo(
	...
	(prevProps, nextProps) => {
		// ⚡ Bolt: Custom comparison to prevent re-renders when selectedDetailNames array
		// has the same content but a different reference.
		return (
			prevProps.isSelected === nextProps.isSelected &&
			prevProps.disabled === nextProps.disabled &&
			prevProps.onToggle === nextProps.onToggle &&
			prevProps.onDetailToggle === nextProps.onDetailToggle &&
			// Deep check for ingredient (essential fields)
			prevProps.ingredient.id === nextProps.ingredient.id &&
			prevProps.ingredient.name === nextProps.ingredient.name &&
			prevProps.ingredient.description === nextProps.ingredient.description &&
			prevProps.ingredient.assetKey === nextProps.ingredient.assetKey &&
			// Deep check for selectedDetailNames
			prevProps.selectedDetailNames.length ===
				nextProps.selectedDetailNames.length &&
			prevProps.selectedDetailNames.every(
				(name, index) => name === nextProps.selectedDetailNames[index],
			)
		);
	},
);
