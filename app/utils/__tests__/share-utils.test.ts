...
import { shareCocktail, canUseWebShare, copyToClipboard } from "../share-utils";
...
describe("share-utils", () => {
	...
	const mockCocktail: Cocktail = {
		...
		instructions: ["ステップ1: AとBを混ぜる。", "ステップ2: 冷やして提供する。"],
		garnish: "レモンツイスト",
		tags: ["クラシック", "簡単"],
		slug: "test-cocktail",
	...
	describe("shareCocktail", () => {
		...
		it("Web Share APIがAbortErrorをスローした場合（ユーザーがキャンセル）、falseを返す", async () => {
			vi.stubGlobal("navigator", {
				share: vi.fn(() => Promise.reject(new DOMException("Aborted", "AbortError"))),
				clipboard: { writeText: vi.fn() },
			});
			const result = await shareCocktail(mockCocktail);
			expect(result).toBe(false);
